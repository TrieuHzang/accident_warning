...
void loop() {
  ...
  if (taiNanDuocXacNhan && !dangCanhBao) {
    ...
    String linkViTri = gps.location.isValid()
      ? "https://maps.google.com/?q=" + String(gps.location.lat(), 6) + "," + String(gps.location.lng(), 6)
      : "Không xác định";

    String thoiGian = gps.time.isValid()
    ...
    Blynk.logEvent("phat_hien_tai_nan", noiDungThongBao);
    Serial.println(noiDungThongBao);
  }
